...
def create_provider_server(zmq_server='tcp://127.0.0.1:7000'):
    context = zmq.Context()
    socket = context.socket(zmq.SUB)
    socket.bind(zmq_server)
    socket.setsockopt(zmq.SUBSCRIBE, initialisation_data['occi.core.id'])
    print zmq_server
    while True:
        data = socket.recv()
        logger.debug(data)

create_provider_server(zmq_server=initialisation_data['attributes']["ocni.cp.dcp_provider_socket"])